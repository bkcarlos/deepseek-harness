...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图——这是与载体无关的服务。有 web 服务器时，它还通过 `/plugins` 提供该文件及其 sourcemap，并转换 index 以注入启动 manifest；没有 web 服务器时（Electron 桌面形态），图与 `clientPath()` 改而为 IPC 桥接服务。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
