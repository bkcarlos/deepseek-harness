...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, and hashes the built bundle into the boot graph — a carrier-agnostic service. With a web server present it also serves each bundle with its source map under `/plugins` and taps the index render to inject the boot manifest; without one (the Electron desktop surface) the graph and `clientPath()` serve the IPC bridge instead. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
