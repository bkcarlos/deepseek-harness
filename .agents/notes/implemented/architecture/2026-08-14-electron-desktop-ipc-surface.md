...
|---|---|
| Embedding the HTTP server and loading `http://localhost` in a webview | Reintroduces the loopback bind and the LAN-trust fence a desktop app never needs; the IPC bridge keeps the same `Response` contract with zero network surface |
| A second, Electron-native GUI reimplementing the roster | Duplicates every `dsh.client` row and the shell; the transport swap reuses the whole web roster |
| `asar: true` packaging | `healProfilesModuleFallback` symlinks `$DSH_HOME/profiles/node_modules` onto real paths, and a symlink into an asar path is not importable |

## Verification

- `packages/client/connection/tests/ipc-api-client.client.spec.ts` drives `IpcApiClient` over a scripted bridge (unary round-trip, SSE streaming, abort semantics, RPC correlation).
