...
flowchart LR
  ...
  pkg_desktop["desktop"]
  svc_desktop["ctx.desktop<br/>Desktop transport bridge"]
  ...
  svc_desktop --> pkg_apiproxy
  svc_desktop --> pkg_connection
  svc_desktop --> pkg_modules
...
| `ctx.desktop` | `core` | `desktop` | - | `apiproxy`, `modules`, `connection` | - | 已就绪宿主服务之上的、与载体无关的桌面桥接：启动图、bundle 字节与 fetch 形态网关复用宿主 RPC 注册表，没有 HTTP 信任栅栏（同机的 IPC renderer 天然即 loopback 受信）。 |
