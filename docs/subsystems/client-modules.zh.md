...
Web 插件表：[dsh-client-modules](../../packages/client/modules) 中 client 模块系统的 Node 半，以 `ctx.clientModules`（`ClientModuleRegistry`）形式提供。它扫描宿主 Loader 的 entry，找出声明了 `dsh.client` 的包，并组合出 `window.__DSH_BOOT__` entry 图——这是与载体无关的服务，不属于 agent loop（智能体循环）主干。它的两个 web 服务器面——在 `/plugins/<id>/client.js` 提供各个 bundle、经 index 转换（index tap）注入启动 manifest（元数据清单）——只在存在 [dsh-host-webserver](../../packages/host/webserver) 时挂载（[web-server.md](web-server.md) 所述的载体提供前缀路由与 index 转换）；Electron 桌面形态则通过其 IPC 桥接读取图与 `clientPath()`。同一个包的浏览器半（`ctx.modules`，即拉取并物化这些 bundle 的 lazy CJS 模块表）属于内核机件，记录在[包 README](../../packages/client/modules/README.md)中，不在本页。
