...
The web plugin table: the Node half of the client module system in [dsh-client-modules](../../packages/client/modules), provided as `ctx.clientModules` (`ClientModuleRegistry`). It scans the host Loader's entries for packages declaring `dsh.client` and composes the `window.__DSH_BOOT__` entry graph — a carrier-agnostic service, not part of the agent-loop spine. Its two web-server faces — serving each bundle at `/plugins/<id>/client.js` and tapping the index render to inject the boot manifest — mount only when a [dsh-host-webserver](../../packages/host/webserver) is present (the carrier in [web-server.md](web-server.md) supplies the prefix route and index tap); the Electron desktop surface instead reads the graph and `clientPath()` through its IPC bridge. The same package's browser half (`ctx.modules`, the lazy-CJS module table that fetches and materializes these bundles) is kernel machinery documented in the [package README](../../packages/client/modules/README.md), not here.
