...
| `ctx.desktop` | `core` | `desktop` | - | `apiproxy`, `modules`, `connection` | - | Carrier-agnostic desktop bridge over the settled host services: boot graph, bundle bytes, and the fetch-shaped gateway reuse the host RPC registry, with no HTTP trust fence (an IPC renderer on the same machine is loopback-trusted). |
